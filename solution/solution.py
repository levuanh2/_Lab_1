...
def format_comparison_table(results: list[dict]) -> str:
    ...
    header = "| {:<40} | {:<40} | {:<40} | {:<10} | {:<10} |".format(
        "Prompt", "GPT-4o Response", "Mini Response", "GPT-4o Lat", "Mini Lat"
    )
    separator = "-" * len(header)

    rows = []
    for r in results:
        rows.append("| {:<40} | {:<40} | {:<40} | {:<10.3f} | {:<10.3f} |".format(
            truncate(r["prompt"], 40),
            truncate(r["gpt4o_response"], 40),
            truncate(r["mini_response"], 40),
            r["gpt4o_latency"],
            r["mini_latency"],
        ))
